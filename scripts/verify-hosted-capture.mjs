...
async function postCapture({ apiUrl, anonKey, accessToken, clientCaptureKey, sourceUrl, sourceText }) {
...
async function main() {
  ...
  const apiUrl =
    ...
    process.env.EXPO_PUBLIC_SHAREBOOK_API_URL ||
  ...
  const sourceUrl = options.url || "https://www.reddit.com/r/reactnative/";
  const sourceText =
    options.text ||
    [
      sourceUrl,
  ...
  if (!capture) throw new Error(`Capture ${clientCaptureKey} was not returned by the hosted API.`);
  ...
  const [runs, reminders, reminderLinks, suggestions] = await Promise.all([
    restRows({
      supabaseUrl,
      serviceRoleKey,
      table: "analysis_runs",
