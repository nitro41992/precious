...
Auth in the app UI is email magic-link + Google OAuth (a Chrome Custom Tab); there is no password sign-in screen. Both return to the app via the `preciouscaptures://auth/callback` deep link. Password users still exist at the Supabase auth level and are how the E2E device suites authenticate without driving those off-app surfaces: mint a session server-side, then hand it to the app via the same callback deep link (`signInWithAuthCallback` in `scripts/lib/e2e-harness.mjs`). Create dogfood/E2E users with `npm run hosted:create-user -- --email ... --password ...`. Android app id is `com.preciouscaptures`. A 404 from the app usually means `capture-intake` isn't deployed for the configured project (`curl -i .../functions/v1/capture-intake` — 401 means deployed, 404 means deploy it).
