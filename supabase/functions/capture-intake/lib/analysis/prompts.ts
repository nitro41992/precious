...
export function buildPrompt(
  ...
  return [
    ...
    "Good review_rationale example: summary 'Looks like a cafe drink to try, so I saved it as Visit and matched Food.' intent 'The capture names a caffeparadiso.nyc coffee drink, which supports Visit.' collections 'Food fits because the capture is about a cafe menu item.' reminder 'No Reminder idea because there is no future date, deadline, or booking window.'",
    "Bad review_rationale examples: 'Suggested Visit because this is an Instagram Reel' is source-format-only; 'The Save Intent is supported' is generic; 'coffee drink to t' is clipped; model, prompt, schema, score, or confidence-percentage wording is debug copy.",
