...
export function buildPrompt(
  ...
  return [
    ...
    "default_intent.rationale must be the same text as field_rationales.purpose.text, including when no Purpose is chosen.",
    ...
    "When you return one Reminder idea, suggested_reminders[0].rationale must be the same text as field_rationales.reminder.text.",
    ...
    "For Purpose, field_rationales.purpose.selection_key must equal default_intent.category. If there is no Purpose, set selection_key to null, selection_label to No intent, and write text as: No intent because [plain reason no concrete action is clear]. Otherwise set selection_label to the Intent label and write field_rationales.purpose.text as: I chose [Intent label] because [plain evidence].",
    "For Collections, field_rationales.collections must contain one entry for each returned collection_decisions item, using the exact collection_id and a concise selection_label for the Collection. Each selected Collection text must read: I picked [Collection label] because [plain evidence]. If no Collection is chosen, return exactly one entry with collection_id null, selection_label No collection, and text as: No collection because [plain reason it does not clearly fit existing collections].",
    "For Later, field_rationales.reminder must mirror the first suggested_reminders item. If there is no Reminder idea, set trigger_value to No Reminder idea, dates and times to null, and text as: No Reminder idea because [plain reason no future date or deadline is clear]. Otherwise set trigger_value to a concise Later label and write text as: I suggested [Later label] because [plain evidence].",
    "For compatibility, copy the exact structured rationale text into default_intent.rationale, each matching collection_decisions[].rationale, and suggested_reminders[0].rationale.",
