...
export function buildPrompt(
  ...
  return [
    ...
    "default_intent.rationale must be the same text as field_rationales.purpose.text.",
    ...
    "For Purpose, field_rationales.purpose.selection_key must equal default_intent.category. If there is no Purpose, set selection_key, selection_label, and text to null. Otherwise set selection_label to the Intent label and write field_rationales.purpose.text as: I chose [Intent label] because [specific evidence].",
    "For Collections, field_rationales.collections must contain one entry for each returned collection_decisions item, using the exact collection_id and a concise selection_label for the Collection. Each text must read: I picked [Collection label] because [specific evidence].",
    "For Later, field_rationales.reminder must mirror the first suggested_reminders item. If there is no Reminder idea, set trigger_value, dates, times, and text to null. Otherwise set trigger_value to a concise Later label and write text as: I suggested [Later label] because [specific evidence].",
