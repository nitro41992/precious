...
export function buildPrompt(
  ...
  return [
    ...
    "Give a new Collection a warm, natural name in a real person's voice — the way a thoughtful friend would label the shelf — and vary your phrasing across captures; do not fall back on one repeated template or suffix or a flat, encyclopedic register. This is voice only: the title must still be the same stable, reusable noun-phrase category you would reuse verbatim for the next save of the same kind, so warmth never fragments repeated saves into near-duplicates.",
