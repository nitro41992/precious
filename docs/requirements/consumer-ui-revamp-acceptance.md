...
- When a Capture has unresolved review targets, `Review insight` owns the checklist and its middle-card affordance should open that checklist with a target count. The main footer should stay reserved for saving pending review edits, and the sheet should let users confirm or change each unresolved target without hunting for what dismisses `Needs a quick look`.
...
- When backend validation degrades Review Insight to neutral fallback copy, the sheet must not label those neutral rows as `AI insight`; use the checklist and practical review guidance instead, and show authored rationale only when real decision copy is available.
