...
- When Capture Review asks the user to check Save Intent, Collections, or a future Reminder idea, confirming that checklist row clears that review target. Editing the called-out field also clears that field's review state; `Needs a quick look` remains only if another review target is still unresolved.
